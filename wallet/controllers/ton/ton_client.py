...
from wallet.config import TON_CLIENT_API_KEY, TON_CLIENT_API_URL, TON_CLIENT_API_URL_PREFIX
...
class TonClient:
    ...
    async def get_by_api(self, url: str, params=None):
        async with aiohttp.ClientSession(base_url=TON_CLIENT_API_URL) as session:
            ...
            response = await session.get(url=f"{TON_CLIENT_API_URL_PREFIX}/{url}", params=params)
